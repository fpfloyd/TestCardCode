...
#               v.0.13       20160930      Added Parser and Locked down Assay Parameters
...
SilverMixingInc = 1200  #Time between Silver Mixing Steps (sec)
...
def assay(theRig):
        ...
        theRig.VibrationStart(MagFlowTime,MagStartFreq,MagEndFreq, MagCycles)
        time.sleep(MagFlowTime+1)
        print ('mag flow time:', str(MagFlowTime+1))
        if Pause == True:
            raw_input('Press enter to continue')
...
def main():
        ...
        if (connect(theRig) or Fakeout):
                ...
                while not done:
                        ...
                        a=raw_input("key to start: (a)ssay, (p)rime, (c)hange folder name, pa(r)se folder, (m)anual operation, (q)uit\n")
