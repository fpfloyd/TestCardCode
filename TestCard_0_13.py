...
def assay(theRig):
        ...
        if param.FillBuff == 'B5':
            # Fill Mixing Chamber with Elyte
            print time.strftime('%H:%M:%S -', time.localtime()), 'Filling Mixing Chamber with 50uL @ 100uL/min'
            theRig.ValveOpen('V1')
            theRig.ValveClose('V3')
            theRig.PumpStart('B5', param.MoveRate, param.MoveVol)
            time.sleep(param.MoveTime)

        if param.FillBuff == 'B4':
            # Fill Mixing Chamber with Elyte
            print time.strftime('%H:%M:%S -', time.localtime()), 'Filling Mixing Chamber with 50uL @ 100uL/min'
            theRig.ValveOpen('V1')
            theRig.ValveClose('V3')
            theRig.PumpStart('B4', param.MoveRate, param.MoveVol)
            time.sleep(param.MoveTime)

        if param.FillBuff == 'B1':
            # Fill Mixing Chamber with Elyte
            print time.strftime('%H:%M:%S -', time.localtime()), 'Filling Mixing Chamber with 50uL @ 100uL/min'
            theRig.ValveOpen('V1')
            theRig.ValveClose('V3')
            theRig.PumpStart('B1', param.MoveRate, param.MoveVol)
            time.sleep(param.MoveTime)
        ...
        if param.DispenseV2 == False:
            #Move to ASV Chamber
            if param.pause == True:
                raw_input('Press Enter to Move Sandwiches')
